...
import { remarkLogModel } from './remark-log-model';

@Component({
  selector: 'app-table-ld',
  templateUrl: './table-ld.component.html',
  styleUrls: ['./table-ld.component.sass']
})
export class TableLdComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (!this.remarkLogs || !this.initialFormData) {
      this.viewService.getRemarkLog(this.u_id, 66, this.u_id).subscribe(res => {
        this.remarkLogs = res;
      })
    }
    console.log("remarkLogs", this.remarkLogs)
  }
